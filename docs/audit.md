...
## PHASE IMPLEMENTATION LOG — KPI Management Feature

**Date:** 2026-08-05  
**Feature:** KPI Management — Superadmin assigns KPIs to Admins; Admin assigns KPIs to Faculty  
**Source:** `docs/issues.md`

### Overview

Extended the existing KPI system to support **scoped, role-based KPI assignment** across three levels:

| Scope | Created By | Visible To |
|-------|-----------|-----------|
| `global` | Superadmin | All Admins system-wide |
| `department` | Superadmin or Admin | All Faculty in a specific department |
| `user` | Superadmin or Admin | A single specific user (Admin or Faculty) |
...
**Files changed:**
- `database/schema.sql` — added `scope ENUM('global','department','user')` and `assigned_to INT FK→users` to `kpi_items`
- `database/migrate_kpi_scope.sql` — **created** — ALTER TABLE migration for existing databases

**New columns on `kpi_items`:**

| Column | Type | Default | Purpose |
|--------|------|---------|---------|
| `scope` | ENUM | `'global'` | Determines visibility breadth |
| `assigned_to` | INT FK (nullable) | NULL | Specific user when scope=user |
...
| File | Change |
|------|--------|
| `api/kpi/save.php` | Rewritten — allows admin role; validates scope, assigned_to, dept ownership |
| `api/kpi/list.php` | Rewritten — role-aware scope filtering; JOINs for `created_by_name`, `assigned_to_name` |
| `api/kpi/delete.php` | Rewritten — ownership check: admin can only delete own KPIs |
| `api/kpi/get.php` | **Created** — GET single KPI by ID with visibility check |
| `assets/js/components.js` | Added **KPI Management** nav link to admin sidebar |

**Scope filtering logic per role:**
- **Superadmin** → sees all KPIs (no filter)
- **Admin** → sees: global KPIs + dept KPIs for their dept + user-KPIs assigned to them + KPIs they created
- **User** → sees: global KPIs with no dept restriction + dept KPIs for their dept + user-KPIs assigned to them

---

### Phase 3 — Superadmin UI

**File changed:** `views/superadmin/settings.php` — KPI tab fully rewritten.

New features:
- **Scope dropdown** in KPI modal (Global / Department / User-Specific)
- **Conditional fields** — "Assign To Admin" or "Department" appear based on scope selection
- **Colored scope badges** in KPI table (🟣 Global, 🔵 Dept, 🟡 User)
- **Assigned To** and **Created By** columns in table
- **Filter bar** — filter KPIs by scope, category, admin, or free-text search
- **All** tab added alongside Core / Strategic / Support
- Edit uses `api/kpi/get.php` for reliable pre-fill

---

### Phase 4 — Admin UI

**File created:** `views/admin/kpi-management.php` — brand new page.

Features:
- 4 stat cards: Total / Created by Me / User-Assigned / From Superadmin
- KPI table with Category, Scope, Assigned To, MFO, Source columns
- Superadmin KPIs shown in purple-tinted rows with 🔒 lock icon (read-only)
- Admin's own KPIs show Edit ✏️ + Delete 🗑️ buttons
- Add KPI modal with Dept-wide / User-Specific scope selection and faculty dropdown
- Filter bar: scope / category / faculty / owner / search
- Category tabs: All / Core / Strategic / Support

---

### Phase 5 — User IPCR Form

**File changed:** `views/users/ipcr-form.php`

Changes:
- `kpi/list.php` call now passes `department_id` explicitly for correct server-side filtering
- `kpiRaw` flat array stored for personal KPI detection
- **Info banner** shown after KPI load: `"X KPI indicators loaded"` + amber badge count if personally assigned
- **Personal assignment tag** 🏷️ amber icon next to MFO text on user-specific rows

---

### Phase 6 — Testing

**File created:** `test/kpi-test-runner.php` — browser-based auto test runner.

11 automated checks:
1. DB schema — `scope` and `assigned_to` columns present
2. Role data — superadmin, admin, user accounts exist
3. Global KPI insert — scope=global, assigned_to=NULL
4. Admin-specific KPI insert — scope=user, assigned_to=admin_id
5. Dept-wide KPI insert — scope=department, department_id set
6. Faculty-specific KPI insert — scope=user, assigned_to=faculty_id
7. Ownership guard — admin blocked from deleting superadmin KPI
8. Cross-dept guard — admin blocked from assigning to foreign-dept faculty
9. API file existence check
10. Admin KPI view file existence
11. API JOIN result includes `created_by_name` and `assigned_to_name`

6 manual browser checks listed on the test runner page.

> ⚠️ Delete `test/kpi-test-runner.php` after testing is complete.

---

*KPI Management feature implementation completed: 2026-08-05*
