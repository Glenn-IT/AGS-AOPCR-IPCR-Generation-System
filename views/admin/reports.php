...
<main class="main-content" id="mainContent">
  <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-2">
    <div>
      <h2><i class="fa-solid fa-file-alt me-2 text-primary"></i>IPCR Reports & Ratings Summary</h2>
      <p>View performance ratings summary, filter, print, and export IPCR reports<span id="deptCaption"></span>.</p>
    </div>
    <div class="d-flex gap-2 no-print">
      ...
      <button class="btn btn-outline-success btn-sm" onclick="exportCSV()"><i class="fa-solid fa-file-excel me-1"></i>Export CSV</button>
    </div>
  </div>
  ...
  <div class="row g-3 mb-3" id="ratingsSummaryCards">
    <div class="col-sm-6 col-xl-3">
      <div class="stat-card">
        ...
        <div class="stat-value" id="summaryDeptAvg">-</div>
        <div class="stat-label">Dept Average Rating</div>
        <div class="stat-trend" id="summaryDeptBadge"></div>
      </div>
    </div>
  ...
  <div class="card mb-3">
    <div class="card-header py-2 d-flex justify-content-between align-items-center">
      ...
      <small class="text-muted">Adjectival performance breakdown</small>
    </div>
...
<script>
  ...
  function applyFilter() {
    ...
    if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="9" class="text-center py-4 text-muted"><i class="fa-solid fa-inbox me-2"></i>No records found.</td></tr>`;
    } else {
      filtered.forEach((f, i) => {
        ...
        tr.innerHTML = `
          <td>${i+1}</td>
          <td><div class="d-flex align-items-center gap-2"><div class="avatar" style="width:30px;height:30px;font-size:0.65rem">${initials}</div><span style="font-size:0.83rem"><strong>${f.user_name}</strong></span></div></td>
          ...
          <td style="font-size:0.83rem;font-weight:700">${rating > 0 ? rating.toFixed(2) : '-'}</td>
  ...
  function updateRatingsSummary(forms) {
    ...
    document.getElementById('summaryDeptAvg').textContent = avg > 0 ? avg.toFixed(2) : '-';
    document.getElementById('summaryDeptBadge').innerHTML = avg > 0 ? getRatingLabel(avg) : '<span class="text-muted">No rated submissions</span>';
  ...
  function exportCSV() {
    ...
    allForms.forEach(f => {
      ...
      rows.push([f.user_name, f.department_name, f.position, f.covered_period, f.date_submitted, r > 0 ? r.toFixed(2) : '-', adj, f.status]);
    });
    const csv = rows.map(r => r.map(v => `"${String(v||'').replace(/"/g,'""')}"`).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'IPCR-Ratings-Summary.csv'; a.click();
    showToast('Report exported!', 'success');
  }

  function printReport() {
    const dept = document.getElementById('deptFilter').value || session.department_name || 'Department';
    ...
    const deptAvg = ratings.length ? (ratings.reduce((a,b) => a+b, 0) / ratings.length).toFixed(2) : '-';
...
<head>
<meta charset="UTF-8">
<title>IPCR Summary & Performance Report — ${esc(dept)}</title>
<style>
...
<div class="header">
  ...
  <h4>SUMMARY OF IPCR RATINGS & PERFORMANCE REPORT</h4>
  <p>Department / Division: <strong>${esc(dept)}</strong> | Date Generated: <strong>${dateStr}</strong></p>
</div>

<div class="summary-box">
  <div class="stat-box"><div class="stat-val">${deptAvg}</div><div class="stat-lbl">Dept Average Rating</div></div>
...
<h5 style="margin-bottom:6px;font-size:9.5pt;font-weight:bold;color:#821131">II. INDIVIDUAL SUBMISSIONS & RATINGS</h5>
<table>
  <thead>
    <tr>
  ...
  <tbody>
    ${rowsHtml || '<tr><td colspan="8" style="text-align:center">No records found.</td></tr>'}
  </tbody>
</table>

<div class="sig-row">
  <div class="sig-box">
    <div class="sig-line">${esc(session.name)}<br><span style="font-weight:normal;font-size:7.5pt;color:#555">Prepared by (Department Head / Admin)</span></div>
  </div>
  <div class="sig-box">
