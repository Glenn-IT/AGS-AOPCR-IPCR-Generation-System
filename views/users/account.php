...
<main class="main-content" id="mainContent">
  <div class="page-header">
    <h2><i class="fa-solid fa-user-cog me-2 text-primary"></i>Account Management</h2>
  ...
  <ul class="nav nav-tabs mb-3" id="accountTabs">
    ...
    <li class="nav-item"><a class="nav-link" href="#logs" data-bs-toggle="tab"><i class="fa-solid fa-list-check me-2"></i>Account Logs</a></li>
  </ul>
...
<script>
  ...
  document.getElementById('newPw').addEventListener('input', function() {
    ...
    bar.style.width = (score * 20) + '%'; bar.className = 'progress-bar ' + (colors[score] || '');
  ...
  async function loadLogs() {
    ...
    if (logs.length === 0) {
      logsTable.innerHTML = '<tr><td colspan="4" class="text-center py-3 text-muted">No activity logs found.</td></tr>';
    } else {
      logs.forEach((log, i) => {
        logsTable.innerHTML += `<tr><td>${i+1}</td><td style="font-size:0.83rem">${log.date}</td><td style="font-size:0.83rem">${log.time}</td><td style="font-size:0.83rem">${log.activity}</td></tr>`;
      });
    }
  }
