...
1. **Timeline Cycle**: Superadmin opens Timeline (`timelines`) $\rightarrow$ IPCR & OPCR forms check `timelines.status = 'open'` and Target Accomplishment Date (`submission_deadline`). Auto-picks Start & End dates based on Academic Year and Rating Period.
...
| `save.php` | POST | `superadmin` | `{id?, academic_year, semester (rating_period), start_date, end_date, submission_deadline / target_accomplishment_date, status}` | `{success: bool, id: int, message: string}` | `views/superadmin/settings.php` |
...
| `timelines` | `id` (INT Auto) $\rightarrow$ `fk_timeline_creator` (`created_by`) | `academic_year`, `semester` (Rating Period), `start_date`, `end_date`, `submission_deadline` (Target Accomplishment Date), `status` (open/closed) | `api/timeline/list.php`, `api/ipcr/save.php`, `api/opcr/save.php`, `api/dashboard/user-stats.php`, `views/superadmin/settings.php` | `api/timeline/save.php`, `setup.php`, `database/seed.php` |
